...
int test_case_1 () {
    ...
    int byte_input[] = {13, 36, 41, 84, 85, 504};    //inputs to mem_count_extfrag()
...
int test_case_2() {
    ...
    int bytes_to_alloc = 7;                                     //size of the first allocated chunk
    int count_inputs[] = {5000, 37, 37, 53, 89, 89, 13, 117};    //inputs for mem_count_extfrag()
...
int test_case_3() {
    ...
    int bytes_to_alloc = 48;                                        //size of the first allocated chunk

    printf("\r\n TEST CASE 3: Testing Reusing Newly Freed Regions\r\n");

    // This for loop allocates 2 chunks of memory that are size 48 byte
    for (int i = 0; i < 2; i++) {
        p[i] = mem_alloc(bytes_to_alloc);
        printf("Starting Address for pointer %d: 0x%x\r\n", i, (U32) p[i]);
    }

    // deallocate one of the 48 byte memory sizes
    mem_dealloc(p[0]);
    ...
    p[2] = mem_alloc(8);
    // should go into the 48 byte memory region that was just deallocated
    printf("Starting Address for pointer %d: 0x%x\r\n", 3, (U32) p[2]);
    if (p[2] != p[0]) {
